...
module NCS
  ...
  class Client
    ...
    def strain_create(strain)
      validate_model(strain, ['name'], NCS.extract_keys(NCS::ROOM_MODEL))
      post(:strains, :create, JSON.generate([strain]))
    end

    def strain_update(strain)
      validate_model(strain, %w[name id], NCS.extract_keys(NCS::ROOM_MODEL))
    ...
    def item_category_create(category)
      validate_model(category, [], NCS.extract_keys(NCS::ITEM_CATEGORY_MODEL))
    ...
    def item_create(item)
      validate_model(item, [], NCS.extract_keys(NCS::ITEM_MODEL))
    ...
    def plant_batches_create_planting(planting)
      validate_model(planting, [], NCS.extract_keys(NCS::PLANT_PACKAGE_MODEL))
      post(:plantbatches, :createplantings, JSON.generate([planting]))
    end

    def plant_batches_create_package(package)
      validate_model(package, [], NCS.extract_keys(NCS::PLANT_PACKAGE_MODEL))
